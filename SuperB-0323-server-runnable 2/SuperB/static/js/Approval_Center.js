...
function handleApproval(action, card) {
    ...
    // 修正模板字符串用法
    fetch(`https://101.200.197.132:8000/update-booking-status/${bookingId}`, {
        method: 'PUT',
...
function fetchFinishedBookings() {
    fetch('https://101.200.197.132:8000/finished-workflow-bookings')
        .then(response => {
...
function fetchPendingBookings() {
    fetch('https://101.200.197.132:8000/pending-bookings')
        .then(response => {
            if (!response.ok) {
