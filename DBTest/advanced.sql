...
BEGIN
    ...
    -- Query user name, room name and equipment info
    SELECT username INTO v_username FROM Users WHERE user_id = NEW.user_id;
    SELECT room_name, equipment INTO v_roomname, v_equipment FROM Rooms WHERE room_id = NEW.room_id;

    IF NEW.status <> OLD.status THEN
        -- Approved status
        IF NEW.status = 'approved' THEN
            SET msg = CONCAT(
                'Booking Confirmed: Your reservation for room ', v_roomname, 
                ' (Equipment: ', IFNULL(v_equipment, 'No special equipment'), 
                ') has been approved. Time: ', NEW.booking_date, 
                ' ', NEW.start_time, ' to ', NEW.end_time
            );
        
        -- Rejected status
        ELSEIF NEW.status = 'rejected' THEN
            SET msg = CONCAT(
                'Booking Rejected: Your reservation for room ', v_roomname, 
                ' was not approved. Original time: ', NEW.booking_date, 
                ' ', NEW.start_time, ' to ', NEW.end_time,
                '. Reason: ', IFNULL(NEW.reason, 'Not specified')
            );
        
        -- Changed status
        ELSEIF NEW.status = 'changed' THEN
            SET msg = CONCAT(
                'Booking Modified: Your reservation for room ', v_roomname, 
                ' has been adjusted. New time: ', NEW.booking_date, 
                ' ', NEW.start_time, ' to ', NEW.end_time,
                '. Change reason: ', IFNULL(NEW.reason, 'Room unavailable')
            );
        
        -- Finished status (new addition)
        ELSEIF NEW.status = 'finished' THEN
            SET msg = CONCAT(
                'Booking Completed: Your reservation for room ', v_roomname, 
                ' (', NEW.booking_date, 
                ' ', NEW.start_time, ' to ', NEW.end_time, 
                ') has ended. Thank you for using our service!'
            );
        
        -- Other statuses...
        ELSEIF NEW.status = 'pending' THEN
            SET msg = CONCAT('Booking Pending: Your reservation for ', v_roomname, ' is awaiting approval');
        ELSEIF NEW.status = 'canceled' THEN
            SET msg = CONCAT('Booking Canceled: You have canceled your reservation for ', v_roomname);
        ELSEIF NEW.status = 'failed' THEN
            SET msg = CONCAT('Booking Failed: ', IFNULL(NEW.reason, 'System error caused booking failure'));
        ELSE
            SET msg = CONCAT('Booking Status Updated: ', NEW.status);
        END IF;
        ...
        INSERT INTO Notifications(
            ...
            CASE NEW.status
                ...
                WHEN 'finished' THEN 'info'      -- Completion notification type
                WHEN 'pending' THEN 'reminder'
                WHEN 'canceled' THEN 'cancellation'
                WHEN 'failed' THEN 'failed'
                ELSE 'info'
            END,
            NOW()  -- Explicit timestamp setting
        );
        
        -- If status is finished, release room resources
...
CREATE EVENT ev_auto_finish_bookings
ON SCHEDULE EVERY 1 MINUTE
DO
BEGIN
    -- 精确到分钟的时间判断（使用TIMESTAMP比较）
    UPDATE Bookings
    SET status = 'finished'
    WHERE status IN ('approved', 'changed')
